 ...
 */
Buffer buildRequest(uint32_t commandCode, uint32_t appId,
...
uint32_t extractResultCode(const Buffer& msg) {
    ...
    while (pos + 8 <= msg.size()) {
        uint32_t avpCode = (static_cast<uint32_t>(msg[pos]) << 24) |
                           (static_cast<uint32_t>(msg[pos + 1]) << 16) |
                           (static_cast<uint32_t>(msg[pos + 2]) << 8) |
                           static_cast<uint32_t>(msg[pos + 3]);
        uint32_t avpLen = (static_cast<uint32_t>(msg[pos + 5]) << 16) |
                          (static_cast<uint32_t>(msg[pos + 6]) << 8) |
                          static_cast<uint32_t>(msg[pos + 7]);

        if (avpLen < 8) break; // malformed

        if (avpCode == 268 && avpLen >= 12) { // Result-Code
            uint32_t rc = (static_cast<uint32_t>(msg[pos + 8]) << 24) |
                          (static_cast<uint32_t>(msg[pos + 9]) << 16) |
                          (static_cast<uint32_t>(msg[pos + 10]) << 8) |
                          static_cast<uint32_t>(msg[pos + 11]);
            return rc;
        }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[ping_client] Connecting to " << host << ":" << port
    ...
    client.setStateCallback(
        [&](ert::diametercomm::Peer::State state) {
            if (state == ert::diametercomm::Peer::State::Open) {
                ...
                Buffer request = buildRequest(cmdCode, config.applicationId,
                                              config.originHost);
                ...
                uint32_t hbh = client.send(
                    std::move(request),
                    [&](const Buffer& answer) {
                        std::cout << "[ping_client] Answer received ("
                                  << answer.size() << " bytes): ";
                        hexDump(answer);
    ...
    // Timeout callback
    client.setTimeoutCallback(
        [&](uint32_t hbh) {
            std::cerr << "[ping_client] TIMEOUT waiting for answer (hop-by-hop="
                      << hbh << ")\n";
            done = true;
            client.close();
        });
